...
**Siguiente paso: empezar la Fase 8 — Nivel avanzado.** La Fase 7 (Nivel intermedio: anclaje de locators) se completó el 2026-08-10 a la primera, 100% interactiva, y cerró un pendiente real del proyecto: el lead 2 de findings §29 (el `.first()` de wishlist sin anclar) pasó de sospecha a **defecto confirmado en vivo y arreglado** — detalle completo en findings §31. El contenido de la Fase 8 no está predefinido; decidirlo con Jorge al empezar. Candidatos vivos, **revisados 2026-08-19** (⚠ los dos primeros de la lista anterior ya NO son candidatos: el click sin bound de la plantilla del Builder se arregló el 2026-08-16 en `03aa765`, y el patrón del drawer §28 entró en el vocabulario del analyzer el mismo día en `368d334` — la lista se quedó sin actualizar cuando aterrizaron; no los re-propongas):

- La **colisión de presupuesto de 150s del fixture** (backlog item 5, findings §33): pequeña, acotada y con la evidencia ya recogida — sigue siendo el mejor candidato "de tamaño ejercicio".
- El **`.first()` sin sondear de `VestidosTallasOverlayPage.isOverlayOpen()`** (lead de cierre de findings §31): es *exactamente* la misma forma de defecto que cerró la Fase 7, pero esta vez sin resolver — el alumno aplicaría el método que ya aprendió (tabla de estados, construir el estado discriminante, no medir con el instrumento sospechoso) a un caso vivo. Ojo con la evidencia: el flake del 2026-08-18 fue un falso NEGATIVO, la forma opuesta a la que §31 predice, así que ni confirma ni refuta.
- El **borde drawer-never de `addToCart`** (backlog item 6a, findings §39): enseña a razonar un *trade-off* real en vez de un arreglo limpio — confirmar por badge y volver antes eliminaría el fallo, pero deja un drawer tardío interceptando el siguiente click (§28).

Material de método disponible, todo con evidencia real en el repo: §34 (un arreglo validado en aislamiento no está validado en el sistema real); §38 y §39 (**dejar el instrumento puesto**: una línea de log añadida una sesión antes contestó, gratis, una pregunta que tres crawls no pudieron — y pasó dos veces en dos días); y §39 otra vez (una corrida **roja** valió más que una verde, porque produjo la evidencia que desbloqueó un ítem). Ojo: el lead P6 **ya no está disponible** como ejercicio — se resolvió el 2026-08-16 —, pero su método sí es material de enseñanza de primera para esta fase: falsar hipótesis offline antes de gastar una corrida en vivo, y entender que **una sonda que NO reproduce también es evidencia** (fue justo lo que descartó a DES y señaló a nuestro propio código).
...
- **Patrón de método en un Page/Component Object para una interacción con estado:** una **consulta** async que nunca lanza (`isXxx(): Promise<boolean>`, con `.catch(() => false)` — ⚠ matizado en la Fase 7: sobre `isVisible()` ese catch solo esconde ambigüedad y se quitó, findings §31; el idioma sigue siendo válido sobre `waitFor`/`waitForURL`/`click({trial})`, que sí lanzan por ausencia) + una **acción** que primero comprueba con la consulta (idempotencia — no repetir la acción si el estado ya es el deseado) y usa esa misma consulta como `verify` de `actUntil`.
