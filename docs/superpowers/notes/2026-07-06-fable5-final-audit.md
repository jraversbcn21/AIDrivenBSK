...
### F3. [Nuevo] Redirect-duplicate pages pay full extraction cost before being discarded — **RESOLVED (2026-07-14)**

> **Resolved 2026-07-14 (direct-TDD, no separate spec/plan cycle — same Order-1 precedent as §3.1; output-identical, so no live re-crawl).** `explorer/crawl/crawler.ts` now dedups the resolved path *early* — right after `acceptConsent`, before `waitForSettle` + aria extraction + `enrichTestIds` — via a new pure, unit-tested helper `isDuplicateResolution(path, alreadyConsidered, markSeen)`. The existing late check is kept as a safeguard for the rare URL-changed-during-settle case (degraded DES shells, findings §7), rewritten to treat both `item.path` and the early `resolvedPath` as already-considered so the early `markSeen` can't make it discard a freshly-extracted valid page (a false-duplicate the naive "keep both checks" form would have hit — caught during brainstorming, not live). `crawler.unit.test.ts` covers all four/five cases (no-redirect / redirect-first / redirect-dup / URL-changed-during-settle ×2). `pnpm test:unit` 263/263, typecheck/lint clean. Output-identical: same paths end up seen, same final extraction list — only *when* the per-page cost is paid changes.
...
### F8. [Nuevo] The act→verify→retry idiom is hand-rolled seven times — **RESOLVED (2026-07-14)**

> **Resolved 2026-07-14 (Jorge's explicit go as part of the maintenance sweep).** `src/support/retry.ts` ships `actUntil({act, verify, deadlineMs, sleepMs, sleep, immediateFirstCheck, onTimeout, now})` — exactly the primitive this finding proposed — and all seven sites now use it (SearchBar's two phases keep their shared 40s deadline; waitForResults keeps its immediate-first-check poll shape; the TemplateGenerator's generated `openOverlay`/`closeOverlay` import the primitive instead of inlining the loop). Behavior-preserving per site, 8 unit tests with an injectable clock. **The mandated full live re-validation pass ran and earned its cost:** `pnpm test` 4/4 + regenerated drafts 5/5, and it exposed a latent M1-era bug — `filter({hasNot: ':disabled'})` never excluded a disabled size button *itself* (has/hasNot match descendants), first observable 2026-07-14 when the top camiseta product's first size went "Coming Soon" [disabled]; fixed with `getByRole(..., {disabled: false})` (verbatim-line git evidence confirms it predates F8).
...
### F10. [Nuevo, menor] `tests/generated/` accumulates stale drafts across sessions — **RESOLVED (2026-07-14)**

> **Resolved 2026-07-14 (direct-TDD, root fix at Jorge's direction).** `pnpm build-tests` now **prunes stale drafts by default** — the whole `tests/generated/` tree is wiped *before* writing the new generation, and only *after* selection has produced ≥1 spec (a failed run never leaves an emptied directory). `--no-prune` opts out for deliberate accumulation. Note the fix inverts this finding's own `--clean`-flag suggestion: an opt-in doesn't fix the root (the default would still accumulate, and the exact predicted failure fired live twice — F18's 23/24 on 2026-07-13, and Jorge's first hands-on `test:generated` on 2026-07-14, where a 2026-07-04 draft failed on the long-fixed B16 strict-mode bug). Live-validated: pruned 50 stale files (25 specs + 25 page objects), fresh regeneration `--top 3` → `pnpm test:generated` **5/5 passed, no retries** — the clean per-generation signal this command exists to give. Unit 363/363, typecheck/lint clean.
...
| 2 | **F3 — done (2026-07-14)** | Small, output-identical, buys crawl-time headroom before any bigger crawl work. Shipped direct-TDD (early resolved-path dedup + pure `isDuplicateResolution` helper); see §2 F3's resolved note. | No |
