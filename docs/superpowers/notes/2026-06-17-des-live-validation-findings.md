...
**Coverage (`pnpm plan --update`, run immediately after the full suite per the §30 rule): 38/139 flows, unchanged from before this effort** (evidence: 23 passed tests). Recorded honestly: the new cart specs' route evidence does not move the number, because the map still holds **0 elements for `/es/shop-cart.html`** (this section's own opening line) — there is no cart flow in the map for the planner to match against, pass or fail. Coverage growth from this effort was never expected to come from the planner's flow-matching; it comes from the suite now exercising code and DES surface (`CartPage`, the cart's own remove/quantity/total controls) that no spec touched before.
