...
**Created:** 2026-06-17. **Last updated:** 2026-08-21.
...
**Suite:** **33 tests, TRUE DESKTOP layout — 8 of them correctness ORACLES** (§40–§42, §45, §46): they assert what DES *should do* (price sort both directions, cart arithmetic, PLP↔PDP price consistency, the discount filter, overlay↔PDP size consistency, card "N COLORES" vs PDP color count, overlay↔PDP size-AVAILABILITY consistency), not what our selectors find. **This is the platform's direction since 2026-08-20 (§40): presence checks cannot catch a product bug, and ~85% of the suite was presence checks.** **The 5 PLP oracles are now diversified across routes (2026-08-21, §44):** the §43 run showed all five sharing one vestidos route, which serialized one SPA bounce into five hard failures — 4 of the 5 now target Hombre Camisas, Hombre Lo Más Vendido, and the two Pantalones PLPs (mujer); only `plp-con-descuento` stayed on vestidos, the one confirmed route with a "Con descuento" filter (Hombre Combo Wins was tried and has none); Pantalones Capri now carries 2 oracles (price-consistency §41 + size-availability §46) — an accepted route-reuse trade-off, design doc 2026-08-21. **Confirmed under a full `pnpm qa-cycle` the same session: run #14, 30/31 passed, 0 flaky, 17.8m — all 5 oracles green, no shared failure.** The one failure was pre-existing, unrelated §26 bounce noise on `sudadera-capucha-cremallera-felpa.spec.ts`, a spec never in scope for this work. **Run #15 (2026-08-21, evening, first with 33 tests): 27/33, degraded window — three §26-bounce failures, no DES bug; scan-loop re-anchor shipped from its evidence (§47).** Previous full run 2026-08-20 (`pnpm qa-cycle`, run #13, pre-diversification): **18 passed / 10 failed / 3 flaky** — three clean layers, none a DES product bug (§43): all five PLP oracles hit §26's SPA bounce on their shared vestidos route (the reason for the diversification above), six tail specs died in ~6s each because **DES went down mid-run** (HTTP 000, probed live, recovered ~10 min later; analyzer classified them `infrastructure`), and three §39-class cart flakes retry-recovered. The healthiest recent baseline is 2026-08-18's **26/26, 0 flaky, 13.0m** (pre-oracles). All flakes are read from their own `error-context.md` before being attributed — standing classes are §28's add-drawer noise and §32's `setQuantity` overshoot.

**Map:** **374 desktop pages / 374 flows**, 0 crawl errors, 13977/13977 unique element ids, checkout present (2026-08-18, §38). Coverage **19/374** (committed; the red run #13 re-annotated 17/374 in `reports/` only — never commit coverage from a red run). Run history **14 runs**. The crawler REPLACES the map rather than accumulating, so breadth is a function of the bounds you pass — the widened recipe (300 pages / 2400000 ms per session) is what produced this one, in ~1h20m.

**Closed and not to be re-opened without new evidence:** backlog P5 (session-invalidation recovery, §32) and P6 (the cold-navigation "defect" that was our own detector believing DES's pre-hydration logged-out header, §33). §34's five bugs are all live-validated, and §35/§38 proved `9a9aade` and `01dd927` in real crawls. **§37's residual multi-add is closed too (§39):** the header cart badge was probed live (empty at 0, reactive without navigation, ~0.9s ahead of the drawer, saturates at `9+`) and now guards `addToCart`'s act, with the click cap demoted to backstop.

**Open, with evidence in their own sections:** the PLP filter/sort clicks never resolve (§38 — a hydration-budget problem, NOT the missing overlay role it was filed as for three crawls; `c8b5544` is untestable-by-crawling, not refuted); `addToCart`'s drawer-never window, which still costs the attempt because the verify stays on the drawer by design (§39); six PLP page objects gating `isLoaded()` on a `filterButton` that takes ~16s to hydrate under a 20s budget (§36); the 150s fixture budget collision (§33, more pressured since §34's evidence pass); a `/ic/` locale leak in the crawler frontier (§38); one unexplained 1-unit jeans line in §37's failing cart that no spec adds. **The five-oracles-on-one-route problem (§43) is CLOSED — see §44:** 4 of the 5 PLP oracles now target distinct routes; only `plp-con-descuento` still runs on vestidos, and that is now a confirmed choice (Hombre Combo Wins has no "Con descuento" filter at all), not an oversight.
...
## Archived sections (§1, §5, §8–§23, §25–§30, §32–§35, §37)
...
| **§26** | Coverage expansion by promoting Builder drafts, suite 8 → 14 (2026-08-04) | Origin of the **§26 SPA-bounce class**: a PLP spec can pass `isLoaded()` and later find itself on the Mujer home (no grid, degraded title) — recovery is `ensureOnPlp()` INSIDE the act, retried every cycle (legitimate because category PLPs ARE server-routable, unlike `/q/`). Also: an **unbounded click starves `actUntil`** (fixed on promotion; the Builder-template copy of the gap was fixed in §34), and the map's own `title` field hardens a Builder loaded-signal with a **prefix** check and zero live probing (anon/auth title variants; degraded-shell `"Bershka \| Bershka"` caught free). |
| **§27** | Footer Component Object (2026-08-04) | Shared chrome ⇒ **Component Object** scoped by accessible name (`contentinfo "Pie de página"`). Deliberately asserts ONLY the landmark — the footer's section inventory is marketing content that reshuffles. `pnpm ask`'s no-match for "footer" is CORRECT (the resolver matches flows; chrome is not a flow) — do not file it as LLM-seam evidence. |
| **§28** | The add-confirmation "watch item" was the detector, not DES (2026-08-04) | **Identify a dialog by its CONTENT, never by count** — dialogs stay mounted while visually closed (§17) and churn concurrently, so a count diff cannot say WHICH dialog appeared; 7 "environment noise" failures were a drawer on screen announcing success. The drawer's close button can be nameless → close via named button, else **Escape**, verified on the content locator. Method note (standing rule since): **read the failure's own `error-context.md` BEFORE re-running** — `test-results/` is overwritten. |
| **§29** | The wishlist spec was structurally incapable of failing (2026-08-06) | **A verify must distinguish "my action worked" from "it was already true"** — assert TRANSITIONS from a guaranteed starting state, not states (shared-account carryover blesses state asserts). `actUntil` swallows the act's error by design, so the whole burden sits on the verify. **A false green is invisible to the entire agentic stack** (no failure → no classification → no healing). Open remnant: one 21.8s run remains unexplained forever — its evidence was destroyed by re-running (§31 makes the unanchored locator plausible but unprovable). |
...
| **§33** | Backlog P6 closed: the "cold-navigation defect" was our own detector (2026-08-16) | **DES serves its server-rendered LOGGED-OUT header for the first ~5-8s of a cold navigation on a perfectly VALID session** — a single-instant read of the tell fires a destructive re-login that cannot succeed. Fix: the tell must **PERSIST** (`SESSION_TELL_CONFIRM_MS = 15_000`, sized 2× the measured window) before recovery believes it. Standing rules minted here: "not found ≠ seen-and-false" only holds when the pre-hydration DOM is genuinely EMPTY; **a probe that does NOT reproduce is evidence** (it eliminated the navigation and pointed at the detector). The 150s fixture budget collision was confirmed REAL here and deliberately left open (pending item 9). |
| **§34** | `/code-review high` on the cart diff — five real fixes (2026-08-16) | Standing rule minted: **retry doctrine must cover a THROWING act, not just a no-op one** — `discoverInteractions()` gave slow-hydrating elements exactly one shot (DES's PLP filter toolbar takes **~16.1s to even exist**, measured). Fixes: `ensureEmptyCart` routed through the session-aware `waitForLoaded()` unconditionally (`0a9b8ca`); Builder template's unbounded click bounded (`03aa765`); §28 drawer message classified `environment-noise` while the `toBe(1)` text was deliberately LEFT as `assertion` — no signature could split drawer noise from a real regression (`368d334`); crawler retry-on-throw (`01dd927`); `primeCart`'s bare single-shot `cartCount()` read polled (`9a9aade` — "0 ≠ genuinely empty"). Also: **live-validating a fix caught the fix's own bug** (an Edit had deleted the drain's remove call). |
| **§35** | The §34 re-crawl: two fixes proven, one defect found in a fix (2026-08-17) | `9a9aade` and `01dd927` proven in a real crawl (checkout seeded first try; filter/sort interactions exist in the map at all for the first time). **Corollary to §28, standing rule:** *when a fix converts a crash into a recorded outcome, check the REASON survives the conversion* — `01dd927`'s bare catch made `outcome:'none'` unable to say throw-vs-no-reveal; `d5f9595` logs the last unresolved click error. Method note: **verify scripts against the real schema before trusting aggregates** — elements/interactions are TOP-LEVEL collections joined by `pageId`, and the element name field is `label`, not `name`. |
| **§37** | One spec add landed 8 units — the addToCart re-click (2026-08-18) | Standing rule minted: **an act→verify→retry loop whose act is DESTRUCTIVE and whose only observable is LAGGY re-fires the destructive act once per cycle of lag** — the anti-double-add guard read the same lagging drawer as the verify, so 8 cycles = 8 real server-side adds (merged into one line by §32's same-product merge, which is what proved a single loop). Fix: `MAX_ADD_CLICKS = 3` cap + `needed N clicks` log so cap residue and §32's Sumar overshoot stay distinguishable (opposite fixes). Superseded-in-part by §39: the badge guard replaced the cap as primary, cap stays as backstop. The 1-unit jeans line in the failing cart remains UNEXPLAINED — if a foreign line reappears in a failure snapshot, that recurrence is the evidence it needs. |
...
## 39. The `addToCart` multi-add start-when fired — the header cart badge probed, and the guard's observable replaced (2026-08-19)

**Context.** The day's `pnpm qa-cycle` (run #12 in the history; suite 22 passed / 2 failed / 2 flaky, 23.3m — a degraded window) produced, for the first time, the exact evidence pairing §37's residual-multi-add item was waiting for (backlog item 6; CLAUDE.md's pending item 7 *at the time* — that local number was reassigned when the item closed, so trust the description, not the number): `add-to-cart.spec`'s failing attempt logged **`addToCart needed 3 clicks before the drawer confirmed`** and then failed its exact-quantity assertion with **`Expected: 1, Received: 2`** — snapshot confirming a healthy session (`Mi cuenta`), the cart page, `Cesta (2)`. That is §37's cap residue demonstrably breaking a spec, which is what the badge probe's cost was gated on. (The run's two hard failures — `hombre/combo-wins` and `mujer/pantalones-combo-wins`, both attempts each — were read from their own `error-context.md` first: both snapshots are the **Mujer home** with no grid, §26's SPA-bounce shape in a degraded window. NOT pending item 6's trigger, which needs `filterButton` failing on the *correct* page in a *healthy* run. Evidence preserved in the session scratchpad before any re-run.)
...
- **Route concentration — CLOSED, see §44.**
...
## 44. The 5 PLP oracles diversified across routes — and a real finding: not every PLP carries the discount filter (2026-08-21)

**Context.** Pending item 1, agreed at the close of the previous session (§43): all five PLP oracles (`plp-precio-ascendente`, `plp-precio-descendente`, `plp-con-descuento`, `plp-pdp-price-consistency`, `plp-tallas-overlay-vs-pdp`) targeted the same vestidos PLP and ran consecutively, so one unstable route (the §26 SPA bounce) serialized into five hard failures in run #13. VPN confirmed active, DES confirmed stable before starting.

**Approach.** The suite already has six maintained PLP page objects beyond vestidos: `HombreCamisasPage`, `HombreComboWinsPage`, `HombreLoMasVendidoPage` (`tests/hombre/pages/`), `PantalonesCapriPlpPage`, `PantalonesComboWinsPlpPage` (`tests/mujer/pages/`). None had the `ensureOnPlp()` recovery method the oracles' `FiltersPanel` calls depend on (§43's `recover` hook) — only `VestidosTallasOverlayPage` did. Added the identical method (open → poll `isLoaded()`, §26/§43's proven shape) to all five. Retargeted four of the five oracle specs, one PLP each:

| Spec | New target |
|---|---|
| `plp-precio-ascendente` | Hombre Camisas |
| `plp-precio-descendente` | Hombre Lo Más Vendido |
| `plp-con-descuento` | Hombre Combo Wins → **reverted, see below** |
| `plp-pdp-price-consistency` | Pantalones Capri (mujer) |
| `plp-tallas-overlay-vs-pdp` | Pantalones Combo Wins (mujer) |

**Live validation, round 1: 4/5 passed first attempt** (`pnpm exec playwright test` against all five specs together) — ascendente (30.7s), descendente (27.2s), pdp-price-consistency (24.1s), tallas-overlay-vs-pdp (24.7s), all green. `plp-con-descuento` against Hombre Combo Wins failed **both** attempts: `FiltersPanel: the filter panel did not open within the deadline`.

**Read the failure's own `error-context.md` before re-running (§28's standing method note) — and it was NOT a bounce.** The snapshot showed a healthy, fully-authenticated page (`button "Mi cuenta"`, no "Iniciar sesión"), the correct heading (`heading "COMBO WINS %"`), and the Filtrar sidebar genuinely **open** (`complementary "Filtros" [active]`). The panel opened fine. What it does not have: a "Con descuento" checkbox. Its full filter set, read directly from the snapshot: Precio ascendente/descendente, Color, Talla, a Precio min/max slider, Novedad, Limpiar — no discount option anywhere, and none of the ~30 visible cards' prices showed a struck-through "Antes" price either (all plain single prices: 25,99 €, 7,99 €, 29,99 €, …).

**Root cause: `FiltersPanel.ts`'s own doc comment premise — "Con descuento (always present, never empty)" — is false in general.** It was true for vestidos (§40-42's probes), not universal. `applyFirstAvailable()`'s verify is a trial-click on the "Con descuento" label text; on a PLP where that label never exists, the verify can never succeed and the method reports "the filter panel did not open" — a misleading diagnostic for "the panel opened, but this PLP has no discount filter," the same class of misdiagnosis §28/§42 already catalogued for a wrong-role or wrong-assumption verify.

**Fix — not a code change, a route choice.** Reverted `plp-con-descuento` to vestidos, the one route where the filter's presence is actually confirmed (§40-42 live probes). Re-ran standalone: **2/2 pass, 27.5s, first attempt.**

**Net result: 4 of the 5 oracles now run on routes distinct from each other and from vestidos; only `plp-con-descuento` remains on vestidos, deliberately.** A future SPA bounce on vestidos can now only fail that one oracle, not all five. `pnpm typecheck` / `pnpm lint` clean throughout.

**Open, for whoever adds the next discount-filter-dependent oracle:** which PLPs actually carry "Con descuento" is unknown beyond this session's two data points (vestidos: yes; Hombre Combo Wins: no). Not worth a dedicated probe on its own — the next time a spec needs that filter, check the target PLP's Filtrar sidebar contents first rather than assuming.

### Validated under a full `pnpm qa-cycle` (run #14, same session, later)

Jorge asked for the full cycle (test → analyze → learn → heal → plan) against the diversified routes, not just the targeted spec runs above. **30/31 passed, 0 flaky, 17.8m** — all 5 oracles green on their new routes (`plp-con-descuento` 40.1s, `plp-pdp-price-consistency` 22.1s, `plp-precio-ascendente` 43.7s, `plp-precio-descendente` 46.2s, `plp-tallas-overlay-vs-pdp` 37.1s), and **none of them shared a failure** — the exact property the diversification set out to buy. `add-to-cart`/`cart-lifecycle`/`checkout-reach` all show `[ProductPage] addToCart needed 2 clicks` (§39's guard working normally) and one clean §33 session-recovery on `cart-lifecycle`, neither a regression.

**The one failure is unrelated to this session's work — pre-existing §26 SPA-bounce noise on a spec that was never in scope.** `mujer/sudadera-capucha-cremallera-felpa.spec.ts` (a §26-era promoted PDP spec, unrelated to the oracle program) failed both attempts on `isLoaded()`'s 20s poll; its own `error-context.md` shows `main` rendering the Mujer home (`Categorías destacadas`, no PDP content) — the same bounce signature the oracle specs used to hit before they gained `ensureOnPlp()`. `SudaderaCapuchaCremalleraFelpaPage` has no such recovery (confirmed by reading the class — plain 3-step `open()`, no `ensureOnPlp()`), and never has; this failure is not a regression from today's changes. Analyzer classified it `assertion/persistent` (the vocabulary gap noted below — this specific bounce shape isn't yet a recognized `environment-noise` signature). Deliberately not fixed here — out of scope for the route-diversification task and pull-based per the platform's operating mode; recorded as a backlog candidate if it recurs.

`pnpm learn` recorded run #14 (14 total in `coverage/run-history.json`). `pnpm heal` found nothing (no selector-drift). **Coverage NOT updated** — `plan --update` was correctly skipped since this was a red run (§30/§32's standing rule); `pnpm plan` (read-only) reports 20/374 evidence-only, not committed.

---

## 45. Color-count oracle probe: PLP "N COLORES" card text vs PDP color listbox — route confirmed on the first card, no fallback needed (2026-08-21, later)

**Context.** Task 1 of a new SDD plan (`.superpowers/sdd/2026-08-21-oracle-colores-card-vs-pdp/`) for a future oracle asserting a PLP card's declared color count matches its PDP's actual color count. Temporary probe `tests/_probe/color-count-probe.spec.ts` (deleted after this section, §18 lifecycle), run live via `pnpm exec playwright test tests/_probe/color-count-probe.spec.ts`, launched detached (`cmd.exe /c ... > log 2>&1` via `Start-Process`, the `pnpm` shim being a `.cmd` — plain `Start-Process -FilePath pnpm` fails with "no es una aplicación Win32 válida"; wrap in `cmd.exe /c`). DES probed HTTP 200 before launch. Result: **2 passed, 1.7m** (`auth.setup` 1.1m + probe 27.3s).

**Round 1 — the FIRST card scanned on Hombre Combo Wins was already multicolor. No fallback to Mujer Camisetas needed (Step 3b not executed).** Literal card text (whitespace-collapsed via `innerText`): `"SUDADERA CAPUCHA CREMALLERA 25,99 € 4 COLORES SUDADERA CAPUCHA CREMALLERA 25,99 € 4 COLORES SUDADERA CAPUCHA CREMALLERA 25,99 € 4 COLORES"` — **the same content appears three times verbatim inside one card's `innerText`**, a genuine markup surprise (not a probe bug): the `<li>` scoped by `a[href*="-c0p"]` apparently wraps three duplicate content nodes (plausibly responsive/breakpoint variants coexisting in the DOM). Consequence for Task 2: a future oracle reading a card's declared color count must not assume `innerText` returns the text once — a naive full-string regex still works here (the pattern repeats identically) but a positional/slice-based read would not. Literal pattern confirmed: uppercase **`"{N} COLORES"`**, directly after the price, single space, no other separator. Other cards sampled in the same viewport (6 of 12 logged after the loop found its target on card 0): 4, 4, 5, 2, 2, 3 COLORES — i.e. the PLP's real range in this sample is 2–5, not a rare edge case.

**Round 2 — PDP `sudadera-capucha-cremallera-c0p234325131.html` (href read from the card, `?colorId=902&stylismId=41`), reached on the first navigation attempt, no retry.** The color selector, read from `div.product-detail-info`'s `ariaSnapshot` (§41's PDP-info scope): a **`listbox "Colores disponibles"`** containing exactly **4** `listitem > option "{ColorName}"` children (`Rosa [selected]`, `Negro`, `Gris`, `Piedra`), each wrapping an `img "{ColorName}"`. **4 options == the card's declared "4 COLORES" — confirms the currently-active/selected color counts as one of the N** (it is a plain listbox member marked `[selected]`, not excluded or rendered outside the list). Locator recommendation for Task 2: `page.getByRole('listbox', { name: 'Colores disponibles' }).getByRole('option')` scoped under `div.product-detail-info`, `.count()` as the PDP-side number — no testId attribute was visible on any of these nodes (`ariaSnapshot` doesn't surface `data-*`; not probed further, role+name is sufficient and is the project's first selector-priority tier anyway).

**`getByRole('radio')` / `getByRole('radiogroup')`: 0 / 0 on this PDP.** The color selector is the ARIA **listbox/option** pattern, not a radiogroup — corrects any radio-based counting assumption for Task 2.

**Hazard confirmed, same class as §31's wishlist repeated-element trap.** `a[href*="colorId"]` matches **17** elements page-wide on this PDP, but only **4** belong to the product itself (href starting with its own `-c0p234325131` path — the same 4 colors as the listbox, same order: Rosa/Negro/Gris/Piedra). The other **13** are `colorId` links on entirely different, unrelated cross-selling products further down the page (a gorra, a camiseta, a pantalón, four other sudaderas — different `-c0p` ids). **A page-wide `colorId`-link count would overcount 4x+ and must never be the color-count source** — Task 2's PDP side must read the scoped `listbox`, never `a[href*="colorId"]` unscoped.
...
- **DOM-level** (no snapshot-string parsing, the shape this probe used for the PDP side): for each `getByRole('button')` in the relevant scope (`overlayDialog` for the overlay, `getByRole('group', { name: /selecciona talla/i })` for the PDP — §42's O5 locator), read `.textContent()` for the size label and `.isDisabled()` for availability. **Recommendation for Task 2, corrected: DOM-level does NOT work for the overlay side** — §42 already measured that the overlay button's `.textContent()` is the literal string `"Talla "`, empty of the size, which lives only in the accessible name; a DOM-level read of the overlay would extract nothing. The actual split used by the shipped spec: **overlay = regex over `ariaSnapshot()`** (the size is only in the name), **PDP = DOM-level** (`.textContent()` + `.isDisabled()`/`aria-disabled`, both agreeing per (c) above).

**Net for Task 2:** route = `PantalonesCapriPlpPage` (existing page object, no new one needed), target product `c0p226931517` (talla 38 of 7 disabled), overlay scope = the same `overlayDialog` locator shape this probe used (`getByRole('dialog').filter({ has: getByRole('button', { name: /^talla /i }) })`), PDP scope = `getByRole('group', { name: /selecciona talla/i })`, availability read = `.isDisabled()` per button on both sides, name-normalize by stripping a leading "Talla " before comparing overlay↔PDP tuples. No "sin stock hoy" fallback path was needed — a genuine stockout was found live on the fourth route tried.

**Task 2 closure (2026-08-21, same session).** Shipped `tests/mujer/plp-tallas-disponibilidad.spec.ts` (oracle #8, suite 32→33). Validated live, 2 standalone passes: **PASS 1.2m, PASS 1.1m** — spec-only, §45's format (~2.4m combined once each run's own `auth.setup` is included) — both first attempt, no retry (`test-results/.last-run.json` confirms `status: passed`, no skip). The scan found a genuinely disabled size both times and the comparison MATCHED — **DES's overlay↔PDP size-availability is consistent**, verified, not merely presence-checked.

---

## 47. Run #15 (first with 33 tests) — the §26 bounce strikes DURING the card scan; recovery moved one loop deeper (2026-08-21, evening)

**Context.** First full `pnpm qa-cycle` with both new oracles (run #15 in the history): **27/33 — 3 failed, 2 environment-noise flakes retry-recovered, 1 skip — 24.5m degraded window.** Every failure evidence-read before classification (§28).

**The three hard failures share ONE mechanism and none is a DES product bug.** All four failure snapshots (`plp-colores-card-vs-pdp` ×2, `plp-pdp-price-consistency` ×2) plus the known §26 victim (`sudadera-capucha`, third consecutive run) show the **home page** — the §26 SPA bounce ejected the specs mid-run. The §43 hardening paid off exactly as designed: every spec died in 22–70s on its own crafted diagnostic (`no card declaring "N COLORES"…`, `no card with a readable price…`), never on the generic 150s timeout.

**The skip counter earned its keep on its first run.** `plp-tallas-disponibilidad` skipped with `no partially out-of-stock product among the 5/6 cards whose overlay opened and parsed` — a **healthy scan, honestly measured**: the morning's out-of-stock talla 38 (§46) was back in stock ~90 min later. Without the counter (the final review's Important #2) this skip would be indistinguishable from a broken route. Corollary recorded: §46's out-of-stock states are **short-lived** — the oracle's opportunism is real, stock rotates within hours.

**The finding: §43's recovery rule applies one loop deeper.** Both failed oracles had `ensureOnPlp` before the scan and inside the PDP-navigation retry loop — but their CARD-SCAN loop had none, so a bounce DURING the scan read empty/carousel cards off the home page with no way back. Notably, the spec that survived the same window (`plp-tallas-disponibilidad`, 78s, healthy 5/6 scan on the same Capri route) is the one whose per-card loop carries `ensureOnPlp` INSIDE it (§42's overlay-open shape) — the discriminating pair, observed live.

**Fix (both specs, same shape):** the scan is now an outer 3-attempt loop with `ensureOnPlp().catch()` at the top of each cycle and a soft count-poll (a bounced page yields 0 and the rescan recovers, instead of the old hard `toBeGreaterThan(0)` that killed attempt 1); an **empty `innerText` breaks the scan immediately** (healthy cards always carry name+price; empty = detached, and burning 5s per remaining dead card would blow the budget). Diagnostics now say `after 3 scans`.

**Validation:** typecheck/lint clean; both specs live standalone **3/3 first attempt (19.7s / 16.8s)** — the window had recovered, so this proves no happy-path regression and zero added cost; **the recovery path itself is unvalidated until the next degraded window** (§26 precedent, stated plainly). Analyzer note: `no card with a readable price` still classifies `unknown` — §43's deliberate vocabulary decision, unchanged.
