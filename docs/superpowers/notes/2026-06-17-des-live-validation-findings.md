...
**Suite:** 26 tests, TRUE DESKTOP layout. Latest full run 2026-08-19 (`pnpm qa-cycle`): **22 passed / 2 failed / 2 flaky** (23.3m, degraded window — both hard failures were §26's SPA bounce, read from their own snapshots, not a defect); the healthiest recent baseline is 2026-08-18's **26/26, 0 flaky, 13.0m**. All flakes are read from their own `error-context.md` before being attributed — the standing classes are §28's add-drawer noise (now `environment-noise` since §34) and §32's `setQuantity` overshoot.
