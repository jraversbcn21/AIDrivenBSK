...
**Created:** 2026-06-17. **Last updated:** 2026-08-06.
...
**The suite tests DES's TRUE DESKTOP layout. Suite is 25 tests; latest full run (2026-08-10) 22/25 passed, 2 flaky (the §28 drawer noise, both retry-recovered), 1 failed (search-plp-pdp both attempts — §7/§24 class in a degraded 15.1m window, analyzer `timeout/persistent`, not chased). The wishlist locators are now ANCHORED to the main product's panel (§31) — §29's lead 2 was confirmed live and fixed.**
...
- **Interaction reliability** is the standing rule (§7): every state-changing interaction must act→verify→retry, every click must be bounded, and every verify must identify *what* it is looking at rather than counting (§28 is the cautionary tale).
...
- **Environment noise is real and documented** (§7): dead `/q/` loads, degraded app shells, DES maintenance pages, and no cart-cleanup fixture. Read §7 before blaming the framework for a red run.
...
1. **One intermediate run passed in 21.8s and is still unexplained.** With the new spec and the broken selector, both code paths should have thrown; the timing leaves no room for a 20s `actUntil` deadline. **The evidence was destroyed by re-running** — `test-results/` is overwritten by the next run, which is exactly §28's own method note, hit live within two sessions of writing it. If this recurs, capture the trace before re-running.
