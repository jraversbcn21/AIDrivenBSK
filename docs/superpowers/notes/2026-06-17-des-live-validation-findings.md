...
**Open watch item (recorded, not chased) — ⚠ ESCALATED 2026-08-04, see §26:** the desktop add-confirmation intermittently does not appear after "Añadir a la cesta" (`ProductPage: no confirmation dialog appeared` — observed 4× on 2026-08-02, plus the first attempt of BOTH full-suite runs on 2026-08-04 = 6 occurrences; always recovered on retry; once with a bounce-to-search snapshot). Same environment-noise family as the mobile Tallas-dialog-close (§14/§16/§18); if it hardens into a pattern, probe whether the drawer is suppressed when the product is already in the cart.
