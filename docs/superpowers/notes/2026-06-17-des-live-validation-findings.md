...
**Task 2 closure (2026-08-21, same session).** Shipped `tests/mujer/plp-tallas-disponibilidad.spec.ts` (oracle #8, suite 32→33). Validated live, 2 standalone passes: **PASS 1.2m, PASS 1.1m** (both first attempt, no retry — `test-results/.last-run.json` confirms `status: passed`, no skip). The scan found a genuinely disabled size both times and the comparison MATCHED — **DES's overlay↔PDP size-availability is consistent**, verified, not merely presence-checked.
