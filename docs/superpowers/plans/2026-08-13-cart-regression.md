...
|---|---|---|
| P1 | Line-item container (role/structure, anchoring ancestor) | **NOT an ARIA `listitem`.** The cart's own product line renders as a set of flat sibling nodes directly under `main` with no wrapping role at all — `main`'s `listitem`s (21) and `article`s (19) belong exclusively to the "Podría gustarte"/"Te puede interesar" recommendation carousel + the 2 gift-ticket/promo-code list items, never the cart line. Real anchoring container (CSS, confirmed via ancestor walk): `div.product-list-card.product-card-full-screen.product-list-card--desktop` → `div.product-list-card__wrapper` → `div.product-list-card__content`, nested in `div.shop-cart__grid` → `div.shop-cart__products` → `div.shop-cart`. A CSS-class scope is a documented deviation (§31 `mainWishlistPanel()` precedent) — there is no role/testid to anchor on instead. |
| P2 | Remove button: accessible name, one per line? confirm dialog after click? | **State-dependent name, confirmed live across 13 consecutive removal clicks.** At line quantity == 1: `button "Eliminar producto"` (trash icon). At quantity >= 2: that button is ABSENT; only `button "Restar unidad"` (decrement) is present, and decrementing to 1 makes "Eliminar producto" reappear in its place. Both share the identical ancestor chain (see P3). **No confirm dialog appeared** on any of the 13 clicks (decrements or the final full removal) — each click's effect was confirmed solely via the header tab's unit count dropping by exactly 1, settling well inside the 20s bound. Practical consequence: a page-wide `getByRole('button', {name:/eliminar producto/i})` finds **zero** matches whenever quantity >= 2 — `CartPage.removeFirstItem()` must target `/eliminar producto\|restar unidad/i` (or decrement-to-1-then-eliminate) instead of the plan's provisional `/eliminar/i`-only literal. |
| P3 | Quantity control: exists? shape (buttons +/− / spinbutton / combobox) + accessible names | **Exists.** Shape: a 2-button stepper with a `status`-role readout between them — **0 spinbuttons, 0 comboboxes** found (confirmed at quantity 1, 2 and 13). Confirmed accessible names: increase = `button "Sumar unidad"` (always present); decrease = `button "Restar unidad"` (present only at quantity >= 2 — see P2 for the quantity==1 swap to "Eliminar producto"). All three controls for a line share one ancestor chain rooted at `div.quantity-selector.product-card-full-screen__quantity-selector`, inside the same `div.product-list-card` container as P1. |
| P4 | Where the per-line quantity is READ from (element + format) | A `status`-role element whose accessible text is the plain integer as a string (observed live: `"1"`, `"2"`, `"13"`) — **not** a spinbutton, so `inputValue()` does not apply; read via `.textContent()`/accessible name instead. Sits between the eliminar/restar button and "Sumar unidad", inside the same `div.quantity-selector` container as P2/P3. |
| P5 | Empty-state signal (exact text/CTA) | Confirmed live verbatim (see §32 dump): heading-ish text **"Cesta vacía"**, body copy "Aún no tienes ningún artículo en la cesta, descubre todo lo que tenemos para ti", CTA `link "Descubrir"` → `/es/h-woman.html`, plus an unrelated "Te puede interesar" recommendations list. Content-identify via `getByText(/cesta vacía/i)` (§28 doctrine — never infer empty from a bare zero-line count, since the mid-load skeleton is also zero lines, see P8). Cross-checked: the header tab independently read 0 at this same state. |
| P6 | Total element (locator + text format, e.g. "Total 119,95 €") | **NOT one combined string** (unlike the mobile §23 capture) — "Total" and its € amount render as SEPARATE sibling text nodes. Label: plain text "Total" inside `span.total-amount-module__title` → … → `div.total-amount-module`. Amount: a separate text node (e.g. "31,98 €", "207,87 €") inside `span.current-price-elem` → `div.price-elem.total-amount-module__price...` → `div.total-amount-module__price-and-touch` → same `div.total-amount-module` ancestor. **`div.total-amount-module` is the shared scoping container for both** (CSS-class deviation, §31 precedent). The plan's provisional `getByText(/total/i)` alone only ever returns the bare "Total" label (no €, so `parseEuroAmount` would get `null`) — confirmed defect, must be fixed in Task 3. Extra hazard confirmed live: once a Subtotal/shipping breakdown renders (seen at 13 units, "Ocultar detalle de costes" [expanded]), a sibling `paragraph "Subtotal"` ALSO matches `/total/i` (`[P6 TOTAL-ish]` returned `["Subtotal","Total"]` in that dump) — another reason to scope to `div.total-amount-module` specifically, which does not contain "Subtotal" (that lives in the sibling `div.sub-total-module`). |
| P7 | Tab "Cesta (N)": counts units or lines? | **Counts UNITS**, confirmed exactly across every observed value: 1 unit → tab "1"; 2 units → tab "2"; 13 units (see §32 environment note) → tab "13"; after full removal → tab "0". Used successfully as the removal loop's own settle signal (13/13 clicks each dropped it by exactly 1). |
| P8 | Mid-load skeleton: what does it expose? (proves `isEmpty()` is false on it) | Confirmed live, identically in both rounds: immediately after `header.goToCart()`, `page.locator('main').ariaSnapshot()` returns literally `- main "Contenido principal"` with **zero children** — no spinner role, nothing. This is indistinguishable from "0 line items" by count alone, confirming `waitForLoaded()`'s either/or design (line items OR the content-identified empty state, never a bare zero-count) is necessary, not over-engineering. |
