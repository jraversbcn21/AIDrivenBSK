...
**Next: decide the next milestone** — check [`2026-07-02-backlog.md`](./2026-07-02-backlog.md) for candidates. **A5** (`add-to-cart.spec.ts` fails deterministically on the current top "camiseta" result, a Personalizable product) is still the most natural next candidate — open and reconfirmed live across three sessions now, unaffected by the audit. **B17** and **F18** are both ⚠ schema/contract-affecting per the audit's own sequencing table — either needs its own spec cycle with live re-validation before implementation, not a quick patch. The audit's "hygiene" grouping (small, independent, zero-behavior-change items — audit §3 row 1) could ride as a companion branch alongside whichever of the above gets picked. Confirm with Jorge before starting new brainstorm/spec work, per the working agreement.
