...
**Next: decide the next milestone** — check [`2026-07-02-backlog.md`](./2026-07-02-backlog.md) for candidates. **B17 is the recommended first task**: now the sole remaining ⚠ schema/contract-affecting item — `makeId()` has no occurrence discriminator, so 830 duplicate element ids / 1,968 excess rows make 32% of the element table redundant, and 127 duplicate instances diverge in their hints. It needs its own spec cycle with live re-validation before implementation (it changes committed element ids wholesale), not a quick patch. The only items left below that class are F3 and F8 (Orders 2 and 5), both small and individually optional. Confirm with Jorge before starting new brainstorm/spec work, per the working agreement.
