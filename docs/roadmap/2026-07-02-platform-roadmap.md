...
| **M8** ✅ | Interaction-aware crawl: the crawler opens a bounded, deduped set of overlays/dialogs during the crawl (`explorer/crawl/interact.ts`); schema 1.4→1.5 adds `interactions[]` + `revealedBy`; Builder excludes revealed elements from loaded-signal selection. Closes backlog B9's "nav menus/overlays opened during crawl" row. Live-validated 2026-07-05: two full crawls confirmed the "Añadir a la cesta → Tallas dialog" mechanism repeatedly (size-selector knowledge invisible to passive crawling, §10); neither crawl's discovery order reached a literal PDP page (pre-existing crawl-order variability, §9/§12/§13, not an M8 defect). `pnpm test` 4/4, `pnpm test:generated` 20/20 | Phase 1/2 knowledge depth | Knowledge |
