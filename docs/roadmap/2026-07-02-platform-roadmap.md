...
**M8b is done — the M9 prerequisite is closed.** The crawl now deterministically captures the "Añadir a la cesta" → "Tallas" dialog interaction: `interactions.mustCapture` (config, default `/^añadir a (la )?cesta/i`) gives matching triggers priority in a new two-pass `selectCandidates`, retried on every page until satisfied once anywhere in the crawl (`InteractionLedger.markSatisfied`/`isSatisfied`/`unsatisfiedPatterns`), with per-product label variants collapsed via `labelClass` and category-PLPs collapsed into one ledger scope via `interactionScope` (also closes two minor findings from M8's final review: chrome dedupe is now genuinely per-crawl-global, and PLP interaction cost dropped accordingly). No schema change (still 1.5). Live-validated 2026-07-05: a 32-page probe captured the interaction on the first attempt; the full 149-page re-crawl landed it in the **committed canonical map** (`coverage/functional-map.json`) — 1 "Añadir a la cesta" → `overlay` interaction with `revealedBy`-tagged Talla elements, verified directly against the file. Findings doc §16. Design: `docs/superpowers/specs/2026-07-05-m8b-deterministic-must-capture-design.md`. Plan: `docs/superpowers/plans/2026-07-05-m8b-deterministic-must-capture.md`.

**M9 (Builder generates interaction specs from `interactions[]`) is now actionable** — its prerequisite (a canonical map that actually contains the capture) is met.

**Two new findings surfaced during M8b's live validation, both confirmed unrelated to M8b's own diff (zero overlap with the files each touches) and left unfixed as out of scope** — see backlog A5/B16: (1) the manual reference spec `tests/cart/add-to-cart.spec.ts` fails deterministically right now because the current top "camiseta" search result is a Personalizable product with a different add-to-cart UI (catalog drift, not flakiness); (2) `builder/select.ts`'s `loadedSignalFor` can pick a testId hint that repeats across an in-page product grid (not shared chrome, not `revealedBy`), causing a Playwright strict-mode violation — a pre-existing latent gap exposed only by this crawl's ordinary discovery-order variance.

**Next: decide the next milestone** — check [`2026-07-02-backlog.md`](./2026-07-02-backlog.md) for candidates: M9 is now unblocked and the most natural next step, but A5/B16 above are also real, live-confirmed bugs. Confirm with Jorge before starting new brainstorm/spec work, per the working agreement.
