...
**Current state (2026-07-05):** M8b (deterministic must-capture interactions) closed — the M9 prerequisite is met. The crawl now guarantees the "Añadir a la cesta" → Tallas-dialog capture (config-driven must-capture trigger list, two-pass priority selection, per-crawl-global `InteractionLedger` with retry-until-satisfied bookkeeping; schema unchanged, 1.5). Live-validated: both a bounded probe and the full 149-page re-crawl captured it, and **the committed canonical map now contains it** — verified directly against `coverage/functional-map.json`. Full detail: roadmap doc's "Where a fresh session resumes", findings doc §16. **M9 is now actionable.** Two new, out-of-scope bugs were found during M8b's no-regression checks (backlog A5, B16) — see "Pending tasks" below. No milestone is currently in flight. **Confirm the next milestone with Jorge before starting brainstorm/spec work.** This line is meant to be replaced wholesale by the next session, not appended to — the roadmap doc is the source of truth for history.

## Pending tasks for next session

1. **No milestone is queued at top priority.** Check `docs/roadmap/2026-07-02-backlog.md` fresh and confirm with Jorge before starting a brainstorm. M9 (Builder generates interaction specs from `interactions[]`) is now actionable — its prerequisite (a canonical map containing the capture) is met. B-NL1 (Phase 9, natural-language instruction interface) is registered but explicitly not actionable — its dependencies (Builder interaction-spec generation, failure triage) don't exist yet.
2. **Backlog A5** (new, found 2026-07-05): `tests/cart/add-to-cart.spec.ts` fails deterministically right now — the current top "camiseta" search result is a Personalizable product with a different add-to-cart UI ("Personalizar"/"Añadir" instead of "Añadir a cesta"). Confirmed unrelated to M8b (zero diff overlap with `src/`/`tests/`). Needs a live probe of the Personalizable flow before fixing.
3. **Backlog B16** (new, found 2026-07-05): `builder/select.ts`'s `loadedSignalFor` can select a testId hint that repeats across an in-page product grid (not shared chrome, not `revealedBy`), causing a Playwright strict-mode violation in a generated spec. Confirmed unrelated to M8b (zero diff overlap with `builder/`/`explorer/extract/`) — a pre-existing latent gap.
