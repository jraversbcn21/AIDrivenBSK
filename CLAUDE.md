...
**Current state (2026-08-06):** Suite **25 tests, TRUE desktop, latest full run 23/25 passed / 2 flaky recovered on retry / 0 failed**. Platform in **use-and-maintain, no open queue item**. Three things landed today, all worth knowing before touching anything:
...
Two open leads from §29, neither blocking: one run passed in 21.8s and is still unexplained (its trace was lost by re-running — §28's own method note, hit live), and an **unverified** suspicion that `.first()` on the wishlist remove-button locator is not anchored to the main product, which would mean `isInWishlist()` answers "is ANY product on this page in the wishlist". Also still open from 2026-08-04: the Builder's interaction template emits an **unbounded click**, so a generated interaction spec hangs to the 150s test timeout instead of failing fast (findings §26). Method note that keeps earning its place: every real bug of the last two sessions came from **reading the failure's own `error-context.md`/trace instead of re-running** — and `test-results/` is overwritten by the next run, so read it first.

3. **Coverage expansion round 2** (findings §30): promoted 10 more Builder drafts (`pnpm build-tests --top 10` → review → promote), suite went **15 → 25 tests**, `pnpm plan` coverage went **26 → 38/139 flows** (best ever). One real mistake made and caught mid-session: `pnpm plan --update` only reflects the *most recent* `pnpm test` invocation's route evidence, not accumulated history — updating the plan after running only the 10 new specs briefly regressed the reported number to 17/139. **Always run the full suite before `pnpm plan --update`**, never a filtered subset. The recurring `add-to-cart` "confirmation drawer never appeared" flake (§28's closed watch item) fired a 3rd time today with the corrected detector in place — worth treating as genuine intermittent DES noise now, not a detection bug, if it keeps recurring. This paragraph is a pointer, not a re-narration; **replace it wholesale next session**, don't append to it.
