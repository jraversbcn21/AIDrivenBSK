...
**Current state (2026-08-16):** Suite **26 tests, TRUE desktop**; latest full run **24 passed / 3 flaky / 0 failed** (19.2m, a slow DES window — every flake read from its own `error-context.md` and attributed to a named, unrelated class). Coverage **38/139 flows**; map **139 desktop pages** with checkout. The cart is now covered end to end — `CartPage`, the `cleanCart` fixture (which closed §7's long-standing cart-cleanup lead) and the `cart-lifecycle` journey spec — and both of its session defects are closed: **P5**, a genuinely invalidated session (§32), and **P6**, which turned out never to be a DES defect at all but our own detector believing DES's pre-hydration logged-out header on a *valid* session (§33). Platform in **use-and-maintain**, no open milestone and no open gate. One narrower item survives P6: pending task 5 below, the 150s fixture budget collision.
...
5. **150s fixture budget collision (spun out of the now-closed P6, 2026-08-16, findings §33)** — `cleanCart` runs as a *fixture*, before a spec's own `test.setTimeout()` applies, and `CartPage.waitForLoaded()`'s worst case equals the des test timeout exactly, so a slow legitimate recovery dies with a bare Playwright timeout instead of the crafted diagnostic. *Start when:* it actually costs a diagnosis — it is diagnostic-quality, not correctness, and far less likely now the P6 misfire is gone. Evidence and both fix directions are in the backlog's item 5.

**Closed, not pending** (detail in the roadmap's history log and the backlog's item entries — deliberately not re-narrated here): the P0 seeded re-crawl (2026-07-30, map commit `3b901bf` — checkout back in the desktop map, `pnpm ask "checkout"` resolves again), the wishlist locator-anchoring lead (2026-08-10, §31), the cart-cleanup fixture (2026-08-13 — `cleanCart`/`ensureEmptyCart`, findings §32; closes the §7 lead), the `CartPage` session-invalidation gap (2026-08-13, findings §32 Task 8 — recovery via the header's logged-out tell, closes backlog **P5**), the `CartPage` cold-navigation lead (2026-08-16, findings §33 — root-caused to our own detector believing DES's pre-hydration logged-out header, closes backlog **P6**), D15 phases 1 & 2, C11 + the runner's Windows-service promotion, `pnpm ask` v1 and v1.1 session-twin grouping, the 2026-07-14 maintenance sweep (F8, F10, F3, CI migration, dead-code sweep), F18, B17, A5, A6.
