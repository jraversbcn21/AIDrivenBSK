...
**Current state (2026-07-05):** M8 is closed and live-validated — the crawler now opens a bounded, deduped set of overlays/dialogs during the crawl (`explorer/crawl/interact.ts`; schema 1.4 → 1.5 adds `interactions[]` and `revealedBy` on elements; `builder/select.ts` excludes revealed elements from loaded-signal selection). Two live crawls (both sessions, interactions on) confirmed the mechanism repeatedly: "Añadir a la cesta" reliably opens the "Tallas" dialog and the analyzer correctly extracts `Talla 32/34/36/38/40/42` + `Descartar`, each tagged `revealedBy` — the exact size-selector knowledge passive crawling could never capture (§10's third classifier bug). Neither crawl's discovery order reached a literal `-c0p` PDP page (0 PDP / mostly PLP in both runs) — pre-existing, documented crawl-order variability (§9/§12/§13), not an M8 defect; the mechanism itself is proven via the functionally-identical Cart-page product-card component. No-regression live: `pnpm test` 4/4 (1 documented flaky retry), `pnpm test:generated` 20/20. Canonical map committed (schema 1.5, 106 pages) on `feat/m8-interaction-aware-crawl`, pending final whole-branch review and merge to `master` — **note: the committed map itself does not contain the Tallas interaction capture** (0 "Añadir a la cesta" interactions, 0 interactions on either Cart page); that capture landed only in gitignored crawl reports from the other two crawls, and a future crawl still needs to land it in the canonical map before M9 can consume it (findings §15). **Next session: confirm with Jorge before picking a milestone** — check `docs/roadmap/2026-07-02-backlog.md` for candidates (B-NL1 is registered as a Phase 9 candidate only, not yet actionable — its dependencies, interaction specs and failure triage, aren't done). This line is meant to be replaced wholesale by the next session, not appended to — the roadmap doc is the source of truth for history.
