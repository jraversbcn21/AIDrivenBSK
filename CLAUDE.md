...
**Current state (2026-08-02):** **The desktop-layout interceptor branch landed** — ⚠ the 2026-07-29 "desktop migration" had left the suite still testing MOBILE (the gender-gate click dropped the `device` param); the real enforcement is now `src/support/layout.ts` (context-level route interceptor + mobile-fingerprint guard on passing tests, installed by the fixtures and `auth.setup`), and task 6 catalogued/fixed the TRUE-desktop selector divergences (suggestion-click search — desktop Enter on `/q/` bounces home client-side; off-canvas filter sidebar; inline PDP size group; "Ver cesta"; the checkout live-session login gate `src/support/loginGate.ts` — DES single-sessions the shared account). Suite **15 tests, live-green on true desktop** (2026-08-04: the wishlist spec from the onboarding Fase 5 repeat, findings §25; 6 promoted Builder drafts — 3 Hombre PLPs, 2 PDP shapes, 1 PLP-card Tallas overlay — findings §26; and a `Footer` component object + navigation spec, findings §27 — note `pnpm ask` correctly answers no-match for chrome like the footer, which is not a flow). Full detail: findings §24's closing subsections. ⚠ The seeded map's checkout abandon-dialog inventory doubt is RESOLVED (desktop entry state IS the shipping chooser — radios + stepper; the map capture was an artifact). **No open queue item — use-and-maintain** (`pnpm qa-cycle`, `pnpm ask`, re-crawl on drift). Open P4 menu gained one confirmed defect (2026-08-04): the Builder's interaction template emits an **unbounded click**, so a generated interaction spec hangs to the 150s test timeout instead of failing fast — root-caused live, findings §26. The add-confirmation watch item is **CLOSED and it was never a DES issue** (findings §28): the drawer appeared every time — `addToCart`'s baseline dialog-COUNT diff simply could not tell *which* dialog it was seeing, so it reported "no confirmation dialog appeared" with the drawer on screen saying "Producto añadido". Fixed by matching the drawer's content; suite then ran **15/15 clean**. Method note worth keeping: all three real bugs found on 2026-08-04 came from reading the failure's own `error-context.md` instead of re-running, and two of them had been sitting behind a "documented environment noise" label — `test-results/` is overwritten by the next run, so read it before retrying. This line is a pointer, not a re-narration; **replace it wholesale next session**, don't append to it.
