...
**Current state (2026-07-14):** Both ⚠ schema/contract-affecting Fable 5 audit items are closed (F18 coverage matching, findings §20; B17 `MapElement.id` dedup, schema 1.7, findings §21). **F3 is now also done** (2026-07-14) — the crawler dedups the resolved path early, before paying the settle wait + aria extraction + `enrichTestIds`, via a pure unit-tested `isDuplicateResolution` helper; direct-TDD, output-identical, no live re-crawl (audit doc §2/§3 F3, backlog "Where a fresh session resumes" #2). No milestone is currently in flight. This line is a pointer, not a re-narration; **replace it wholesale next session**, don't append to it. The roadmap doc's "Where a fresh session resumes" section is the source of truth for history.
...
2. **Candidates, in the audit's own §3 sequencing order** (`docs/superpowers/notes/2026-07-06-fable5-final-audit.md` §2–3): **F3 is done** (2026-07-14). **F8** (centralize the act→verify→retry idiom, hand-rolled 7×, into one `src/support/retry.ts` primitive) is next in sequence but explicitly ⚠-rated — "touches every live-validated interaction path... human call on whether the consolidation is worth it now," not a default pick. Beyond the audit's cleanup items, the larger open direction is advancing to **Phase 6** (Failure Analyzer / Risk Analysis Agent).
