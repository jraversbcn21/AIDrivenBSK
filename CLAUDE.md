...
**Current state (2026-07-05):** M8 (interaction-aware crawl) closed and merged to `master` (`c6bdf79`) — the crawler now opens overlays/dialogs during the crawl (schema 1.5 adds `interactions[]`/`revealedBy`; Builder excludes revealed elements from loaded-signal selection). Mechanism proven live twice ("Añadir a cesta" → Tallas dialog reveal), but **the committed canonical map does not itself contain that capture** (crawl nondeterminism) — see "Pending tasks" below. Full detail: roadmap doc's "Where a fresh session resumes", findings doc §15. No milestone is currently in flight. **Confirm the next milestone with Jorge before starting brainstorm/spec work.** This line is meant to be replaced wholesale by the next session, not appended to — the roadmap doc is the source of truth for history.

## Pending tasks for next session

1. **No milestone is queued at top priority.** Check `docs/roadmap/2026-07-02-backlog.md` fresh and confirm with Jorge before starting a brainstorm. B-NL1 (Phase 9, natural-language instruction interface) is registered but explicitly not actionable — its dependencies (Builder interaction-spec generation, failure triage) don't exist yet.
2. **If M9 (Builder generates interaction specs from `interactions[]`) is picked:** it needs a canonical map that actually contains an "Añadir a cesta" → Tallas-dialog interaction first. The current committed map has zero — re-run `pnpm explore --update` (repeat if needed; the capture is nondeterministic per findings §15) until one lands, or make the capture deterministic (e.g. a must-capture trigger list, or label-prefix equivalence classing like `Añadir a la cesta *` in the dedupe ledger — see the M8 final review's Recommendations).
3. Lower-priority backlog items, unordered: **C11** (GitLab e2e runner reachability to `*.inditex.grp`, never confirmed), **C13** (flaky-test tagging for the future Reporting Agent, only partially addressed), **D15** (checkout/payment flows — highest-risk, untested, DES pre-prod only).
4. Minor, non-blocking items surfaced by M8's final review (not urgent, good input if touching `explorer/crawl/interact.ts` again): chrome-trigger dedupe is per-session not per-crawl-global as the design spec states; `routePattern` doesn't collapse distinct category PLPs, so per-page interaction cost is ~10x the design's original estimate.
