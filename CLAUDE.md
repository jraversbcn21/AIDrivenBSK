...
**Current state (2026-07-04):** B14 is closed and shipped — shared-element deprioritization (`component` provenance on Header/Footer/MiniCart elements, schema 1.4, pass-major `loadedSignalFor`) is live-validated: 17/17 generated specs pass, including a leaf page with no testId-bearing element that now picks a page-specific role signal (a PDP carousel control) instead of the generic shared header button. Merged to `master` (fast-forward, `17b6b9e..b0e7a0e`), feature branch deleted, pushed to `origin/master`. **A4 (backlog cosmetic naming debt) is also closed** — `src/components/MiniCart.ts` renamed to `CartTab.ts` (`Header.miniCart()` → `cartTab()`, `Header.openMiniCart()` → `goToCart()`); `typecheck`/`lint`/`test:unit` all pass. Working tree is clean; no milestone in flight. **Next session: confirm with Jorge before picking a milestone** — the only queued candidate is **M8** (interaction-aware map knowledge — open nav menus/overlays during crawl; backlog B9; needed before the Builder can generate anything beyond navigation specs). No other candidates are queued; if Jorge wants something else, re-check `docs/roadmap/2026-07-02-backlog.md` for lower-priority items first. This line is meant to be replaced wholesale by the next session, not appended to — the roadmap doc is the source of truth for history.
