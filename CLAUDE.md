...
**Current state (2026-08-13):** Suite **26 tests, TRUE desktop**; latest full run **25 passed / 1 flaky / 0 failed**, 17.5m (the one flake, `cart/add-to-cart.spec` attempt 1, retry-recovered and confirmed via its own failure snapshot to be the pre-existing §28 drawer-noise class, unrelated to session handling). Coverage **38/139 flows** (unchanged — the map's two Cart-type flows were already covered via the checkout specs before this session, so pass/fail of the cart specs couldn't move the number; the map's 0-element count for `/es/shop-cart.html` is a separate, still-true fact about crawl depth, not about flow coverage — findings §32); map **139 desktop pages** with checkout. This session's cart-regression effort shipped `CartPage`, `parseEuroAmount`, `cart-lifecycle.spec`, the `cleanCart`/`ensureEmptyCart` fixture (closes the §7 cart-cleanup lead for a live session), found + fixed three pre-existing production defects live (`CartPage.setQuantity` click-retry overshoot, `CartPage.removeFirstItem` stray re-click + verify gap, `ProductPage.addToCart`'s page-wide-unanchored add button — §29's "false green invisible to the stack" thesis a third time), and shipped `CartPage`'s session-invalidation recovery (closes backlog P5 — `CartPage.waitForLoaded()` now detects the header's logged-out tell and re-authenticates, bounded to one attempt, findings §32 completion Task 8). Platform in **use-and-maintain, one open item**: a second, narrower `CartPage` defect found live during Task 8's own validation — a fresh cart navigation immediately after `auth.setup` (no test in between) can render `/es/member-hub.html` content instead, with a genuinely valid session — reproduced 2/2 in that specific standalone ordering but not in the full suite; recorded, not chased (pending task 5, below, replaces the now-closed session-invalidation item).
...
4. **Low-priority, unordered:** the Builder interaction-template's **unbounded click** (confirmed defect, findings §26 — the only item here with its evidence already in hand); **analyzer vocabulary gap (new 2026-08-10)** — the §28 drawer-noise message (`confirmation drawer never appeared`) falls through to `unknown` because no classifier pattern matches it, so `heal` never sees it; **desktop PLP filter/sort viewport gap** — the crawler can't click PLP filter/sort controls on desktop (outside-viewport/timeout skips, findings §24), so PLP overlay knowledge is thinner than mobile's; checkout draft generation vs the Builder's `CHECKOUT_ROUTE` guard (needs `test.skip(!env.checkoutAllowed)` in the template first — detail in the backlog's §D); per-path interaction disable for seeded checkout crawls + `tramitar` in the destructive regex (pairs with the previous item); D15 phase 3 (payment-step capture — hypothetical, parked); the F18 plan-wording nit.
5. **`CartPage` cold-navigation wrong-page gap (new 2026-08-13, findings §32 completion Task 8)** — a fresh cart navigation as the very first post-`auth.setup` action (nothing else running in between) can render `/es/member-hub.html` content instead of the cart, with the degraded-shell title `"Bershka | Bershka"` (§7/§13) — the session is genuinely VALID (header shows `"Mi cuenta"`/`"Cerrar sesión"`), so this is NOT the (now-closed) session-invalidation gap. Reproduced 2/2 in a standalone `pnpm exec playwright test tests/cart/cart-lifecycle.spec.ts` run; did NOT reproduce in the full suite or the `tests/auth tests/cart` reproduction recipe. Hypothesis, unconfirmed: a timing/propagation race (DES backend session-association, or the SPA's own client-side bootstrap reading a persisted "last route" from `storageState`'s captured localStorage) specific to zero elapsed time since `auth.setup`. *Evidence that would start it:* it already exists (deterministic 2/2), but a live discriminating probe is needed before any fix — do not guess at the mechanism. Fix direction: TBD pending that probe; candidates include a settle wait before the first cart navigation in an invocation, or extending `CartPage`'s existing recovery to a "valid session, wrong content" branch.

**Closed, not pending** (detail in the roadmap's history log and the backlog's item entries — deliberately not re-narrated here): the P0 seeded re-crawl (2026-07-30, map commit `3b901bf` — checkout back in the desktop map, `pnpm ask "checkout"` resolves again), the wishlist locator-anchoring lead (2026-08-10, §31), the cart-cleanup fixture (2026-08-13 — `cleanCart`/`ensureEmptyCart`, findings §32 completion; closes the §7 lead for a live session), the `CartPage` session-invalidation gap (2026-08-13, findings §32 completion Task 8 — `CartPage.waitForLoaded()` now recovers via the header's logged-out tell, closes backlog P5; superseded by the new, narrower item 5 above, which is a different mechanism), D15 phases 1 & 2, C11 + the runner's Windows-service promotion, `pnpm ask` v1 and v1.1 session-twin grouping, the 2026-07-14 maintenance sweep (F8, F10, F3, CI migration, dead-code sweep), F18, B17, A5, A6.
