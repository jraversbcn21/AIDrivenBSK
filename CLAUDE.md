...
**Current state (2026-08-02):** **The desktop-layout interceptor branch landed** — ⚠ the 2026-07-29 "desktop migration" had left the suite still testing MOBILE (the gender-gate click dropped the `device` param); the real enforcement is now `src/support/layout.ts` (context-level route interceptor + mobile-fingerprint guard on passing tests, installed by the fixtures and `auth.setup`), and task 6 catalogued/fixed the TRUE-desktop selector divergences (suggestion-click search — desktop Enter on `/q/` bounces home client-side; off-canvas filter sidebar; inline PDP size group; "Ver cesta"; the checkout live-session login gate `src/support/loginGate.ts` — DES single-sessions the shared account). Suite **7/7 on true desktop**, generated 5/5. Full detail: findings §24's closing subsections. ⚠ The seeded map's checkout abandon-dialog inventory doubt is RESOLVED (desktop entry state IS the shipping chooser — radios + stepper; the map capture was an artifact). **No open queue item — use-and-maintain** (`pnpm qa-cycle`, `pnpm ask`, re-crawl on drift). Open P4 menu unchanged, plus a new watch item: the desktop add-confirmation drawer intermittently doesn't appear (4× on 2026-08-02, retry always recovered — findings §24 watch item). This line is a pointer, not a re-narration; **replace it wholesale next session**, don't append to it.
