...
**Current state (2026-08-13):** Suite **26 tests, TRUE desktop**; latest full run **24 passed / 2 failed** (`cart/add-to-cart.spec`, `cart/cart-lifecycle.spec`, both attempts each — a newly-found `CartPage` session-invalidation gap, findings §32 completion; NOT one of the general §7/§28 noise classes, NOT YET FIXED). Coverage **38/139 flows** (unchanged — the map's two Cart-type flows were already covered via the checkout specs before this session, so pass/fail of the new cart specs couldn't move the number; the map's 0-element count for `/es/shop-cart.html` is a separate, still-true fact about crawl depth, not about flow coverage — findings §32); map **139 desktop pages** with checkout. This session's cart-regression effort shipped `CartPage`, `parseEuroAmount`, `cart-lifecycle.spec`, and the `cleanCart`/`ensureEmptyCart` fixture (closes the §7 cart-cleanup lead for a live session), and found + fixed three pre-existing production defects live (`CartPage.setQuantity` click-retry overshoot, `CartPage.removeFirstItem` stray re-click + verify gap, `ProductPage.addToCart`'s page-wide-unanchored add button — §29's "false green invisible to the stack" thesis a third time). Platform in **use-and-maintain, one open item**: `CartPage` has no session-recovery path equivalent to checkout's `loginGate.ts`, so it fails deterministically when it runs after `login.spec` invalidates the shared session (pending task 5, below).
...
5. **`CartPage` session-invalidation gap (new 2026-08-13, findings §32 completion)** — confirmed defect, not speculative: `add-to-cart.spec`/`cart-lifecycle.spec` fail both attempts, deterministically, whenever they run (via the `cleanCart` fixture's `CartPage.waitForLoaded()`) after `login.spec`'s mid-suite re-auth has invalidated the shared session (§24's documented mechanism) — `CartPage` has no equivalent to checkout's `src/support/loginGate.ts` in-dialog re-login. First observed in Task 7's full-suite validation (never exercised in full-suite context by Tasks 1–6, which validated the new cart specs standalone only). *Evidence that would start it:* it already exists — reproduced 2/2 in the discovering run. Fix direction: extend `loginGate.ts`'s pattern (or an equivalent degraded-render detector) to `CartPage`/`ensureEmptyCart`.

**Closed, not pending** (detail in the roadmap's history log and the backlog's item entries — deliberately not re-narrated here): the P0 seeded re-crawl (2026-07-30, map commit `3b901bf` — checkout back in the desktop map, `pnpm ask "checkout"` resolves again), the wishlist locator-anchoring lead (2026-08-10, §31), the cart-cleanup fixture (2026-08-13 — `cleanCart`/`ensureEmptyCart`, findings §32 completion; closes the §7 lead for a live session — session-recovery under an invalidated session is the separate, still-open item 5 above), D15 phases 1 & 2, C11 + the runner's Windows-service promotion, `pnpm ask` v1 and v1.1 session-twin grouping, the 2026-07-14 maintenance sweep (F8, F10, F3, CI migration, dead-code sweep), F18, B17, A5, A6.
