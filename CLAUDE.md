...
**Current state (2026-07-30, session closed):** **Explorer desktop migration COMPLETED** (commit `b685905`) — the canonical map is now **DESKTOP-layout knowledge**: 140 pages / 140 flows, 0 errors, 42 PDPs (best ever), B17 clean, must-capture Tallas in the map, 0 mobile-drawer elements; suite 7/7, `plan --update` **10/140 covered (best ever)**, generated specs 5/5 live (findings §24 completion note). **NEXT SESSION'S FIRST ACTION (Jorge's decision, P0 in the backlog): the SEEDED re-crawl** — the 2026-07-30 crawl ran without `EXPLORER_SEED_CHECKOUT=on`, so the desktop map has **no Checkout flow** and `pnpm ask "checkout"` answers no-match. Recipe + gates: backlog resume section item P0 (PowerShell env vars, launch DETACHED via `Start-Process` — the agent-harness kills background tool tasks at ~40 min; `primeCart` needs a non-empty cart and DES purges it server-side). Checkout specs themselves stay green regardless. Also filed: desktop PLP filter/sort viewport gap (backlog P4). This line is a pointer, not a re-narration; **replace it wholesale next session**, don't append to it.
...
0. **⚠ P0 — SEEDED RE-CRAWL (already decided by Jorge, 2026-07-30 — the one item that is a queue, not a menu):** restore the Checkout flow to the desktop map with `EXPLORER_SEED_CHECKOUT=on` (~40 min, next healthy-DES window). Full recipe + verification gates in the backlog's P0 entry. Until it runs, `pnpm ask "checkout"` answers no-match (the checkout *specs* stay green — they walk the UI path).
