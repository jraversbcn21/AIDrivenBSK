...
1. **Start here:** **the Phase 9 exit gate is the blocking item — Jorge's review of `docs/superpowers/notes/2026-07-14-phase9-decision-log.md`.** At that review, two decisions are due: (a) whether B-NL1 completes Phase 9 or stands as its own milestone, and (b) the model routing policy re-evaluation (it was neutralized "until Phase 9 completes"). **B-NL1** (NL instruction interface) is the next candidate milestone, needing its own scope round (matching mechanism, ambiguity handling, surface — backlog §E). Read `docs/roadmap/2026-07-02-backlog.md`'s "Where a fresh session resumes" section fresh (it's the live, authoritative list — don't rely on the summary below if the two have drifted).
