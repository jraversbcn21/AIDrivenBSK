...
**Current state (2026-08-02):** **The desktop-layout interceptor branch landed** — ⚠ the 2026-07-29 "desktop migration" had left the suite still testing MOBILE (the gender-gate click dropped the `device` param); the real enforcement is now `src/support/layout.ts` (context-level route interceptor + mobile-fingerprint guard on passing tests, installed by the fixtures and `auth.setup`), and task 6 catalogued/fixed the TRUE-desktop selector divergences (suggestion-click search — desktop Enter on `/q/` bounces home client-side; off-canvas filter sidebar; inline PDP size group; "Ver cesta"; the checkout live-session login gate `src/support/loginGate.ts` — DES single-sessions the shared account). Suite **14 tests, live-green on true desktop** (2026-08-04: the wishlist spec from the onboarding Fase 5 repeat, findings §25, plus 6 promoted Builder drafts — 3 Hombre PLPs, 2 PDP shapes, 1 PLP-card Tallas overlay — findings §26; `add-to-cart` is reliably flaky-on-first-attempt, see the watch item below). Full detail: findings §24's closing subsections. ⚠ The seeded map's checkout abandon-dialog inventory doubt is RESOLVED (desktop entry state IS the shipping chooser — radios + stepper; the map capture was an artifact). **No open queue item — use-and-maintain** (`pnpm qa-cycle`, `pnpm ask`, re-crawl on drift). Open P4 menu gained one confirmed defect (2026-08-04): the Builder's interaction template emits an **unbounded click**, so a generated interaction spec hangs to the 150s test timeout instead of failing fast — root-caused live, findings §26. Watch item ⚠ escalated: the desktop add-confirmation drawer intermittently doesn't appear — now 6 occurrences (4× 2026-08-02, plus the first attempt of both full-suite runs on 2026-08-04); retry always recovers, but it is a pattern now, not noise (findings §24/§26). This line is a pointer, not a re-narration; **replace it wholesale next session**, don't append to it.
