...
describe('parsePlanArgs', () => {
  ...
  it('parses flags', () => {
    const a = parsePlanArgs(['--update', '--map', 'm.json', '--evidence', 'e.json', '--history', 'h.json', '--top', '3']);
    expect(a).toEqual({ update: true, map: 'm.json', evidence: 'e.json', history: 'h.json', top: 3 });
  });
